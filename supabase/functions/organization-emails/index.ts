...
interface EmailRequest {
  type: 'invitation' | 'approval' | 'rejection' | 'transfer' | 'welcome' | 'member_approval' | 'overdue_reminder';
  to: string;
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { type, to, organizationName, token, adminMessage, memberName, transferDetails, invoiceData }: EmailRequest = await req.json();
