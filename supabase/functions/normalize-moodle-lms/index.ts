...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Starting normalization of Moodle LMS entries...');

    // First, get count of records to update
    const { count, error: countError } = await supabaseClient
      .from('organizations')
      .select('*', { count: 'exact', head: true })
      .ilike('learning_management', '%moodle%')
    ...
    // Update all organizations with any Moodle variant in learning_management to "Moodle"
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Normalized ${count || 0} organizations to "Moodle"`,
        count: count || 0
      }),
      { 
    ...
    console.error('Error in normalize-moodle-lms:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
        error: error.message 
