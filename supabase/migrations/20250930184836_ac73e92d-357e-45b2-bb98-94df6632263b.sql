-- Update handle_new_user function to include new VoIP and Network Infrastructure fields
...
BEGIN
  ...
  -- Insert profile with all fields including VoIP and Network Infrastructure
  INSERT INTO public.profiles (
    ...
    admissions_crm, alumni_advancement_crm, payment_platform, meal_plan_management,
    ...
    other_software_comments, is_private_nonprofit, voip, network_infrastructure
  )
  VALUES (
    NEW.id,
    ...
    COALESCE(NEW.raw_user_meta_data->>'payment_platform', ''),
    COALESCE(NEW.raw_user_meta_data->>'meal_plan_management', ''),
    COALESCE(NEW.raw_user_meta_data->>'identity_management', ''),
    COALESCE(NEW.raw_user_meta_data->>'door_access', ''),
    COALESCE(NEW.raw_user_meta_data->>'document_management', ''),
    ...
    COALESCE(NEW.raw_user_meta_data->>'voip', ''),
    COALESCE(NEW.raw_user_meta_data->>'network_infrastructure', '')
  )
  ...
  IF organization_name != '' AND organization_name IS NOT NULL THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.organizations WHERE name = organization_name) THEN
      ...
      INSERT INTO public.organizations (
        ...
        payment_platform, meal_plan_management, identity_management, door_access, document_management,
        primary_office_apple, primary_office_asus, primary_office_dell, primary_office_hp,
        primary_office_microsoft, primary_office_other, primary_office_other_details,
        other_software_comments, membership_status, voip, network_infrastructure
      )
      VALUES (
        organization_name,
        new_profile_id,
