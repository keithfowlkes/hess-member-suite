-- Drop the existing trigger first, then recreate everything
DROP TRIGGER IF EXISTS refresh_analytics_on_org_change_sql ON public.organizations;
DROP FUNCTION IF EXISTS public.refresh_analytics_datacube_sql() CASCADE;
...
BEGIN
  ...
  INSERT INTO system_analytics_datacube (system_field, system_name, institution_count)
  SELECT field, value, cnt::integer FROM system_counts;
