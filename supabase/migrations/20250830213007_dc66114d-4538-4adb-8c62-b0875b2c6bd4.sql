-- Create missing organization records, avoiding duplicates by name
...
WHERE p.organization IS NOT NULL 
  ...
  AND NOT EXISTS (
    ...
    WHERE o2.name = p.organization
  );

-- For existing organization names, update the contact_person_id to the most recent profile
UPDATE organizations 
SET contact_person_id = (
  SELECT p.id 
  FROM profiles p 
  WHERE p.organization = organizations.name 
    AND NOT EXISTS (
      SELECT 1 FROM organizations o2 
      WHERE o2.contact_person_id = p.id
    )
