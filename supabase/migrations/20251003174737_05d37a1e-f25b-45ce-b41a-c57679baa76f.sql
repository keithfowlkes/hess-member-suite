-- Drop the cross-access function and policies
...
DROP FUNCTION IF EXISTS public.cohort_leader_can_access_org;

-- Create a simpler function that only checks exact cohort matches
...
BEGIN
  ...
  -- Check if any of the organization's systems match the user's cohorts (exact match only)
  RETURN (
