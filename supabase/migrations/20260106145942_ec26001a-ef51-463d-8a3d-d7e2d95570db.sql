-- Update the refresh_analytics_datacube_sql function to exclude Administrator accounts
...
BEGIN
  ...
  -- Count ONLY member organizations, excluding Administrator accounts
  ...
  -- Insert system counts with ON CONFLICT handling (excluding Administrator accounts)
  INSERT INTO system_analytics_datacube (system_field, system_name, institution_count)
  ...
  FROM (
    ...
    FROM organizations 
    WHERE membership_status = 'active' AND organization_type = 'member' AND name NOT ILIKE '%Administrator%'
    ...
    FROM organizations WHERE membership_status = 'active' AND organization_type = 'member' AND name NOT ILIKE '%Administrator%'
  ...
  -- Insert hardware counts with ON CONFLICT handling (excluding Administrator accounts)
  INSERT INTO system_analytics_datacube (system_field, system_name, institution_count)
  SELECT 'primary_office_hardware', hardware_name, cnt::integer
  FROM (
    SELECT 'Apple' as hardware_name, COUNT(*) as cnt
    FROM organizations WHERE membership_status = 'active' AND organization_type = 'member' AND name NOT ILIKE '%Administrator%' AND primary_office_apple = true
    UNION ALL
    SELECT 'Lenovo', COUNT(*)
    FROM organizations WHERE membership_status = 'active' AND organization_type = 'member' AND name NOT ILIKE '%Administrator%' AND primary_office_lenovo = true
    UNION ALL
    SELECT 'Dell', COUNT(*)
    FROM organizations WHERE membership_status = 'active' AND organization_type = 'member' AND name NOT ILIKE '%Administrator%' AND primary_office_dell = true
    UNION ALL
    SELECT 'Hp', COUNT(*)
    FROM organizations WHERE membership_status = 'active' AND organization_type = 'member' AND name NOT ILIKE '%Administrator%' AND primary_office_hp = true
    UNION ALL
    SELECT 'Microsoft', COUNT(*)
    FROM organizations WHERE membership_status = 'active' AND organization_type = 'member' AND name NOT ILIKE '%Administrator%' AND primary_office_microsoft = true
    UNION ALL
    SELECT 'Other', COUNT(*)
    FROM organizations WHERE membership_status = 'active' AND organization_type = 'member' AND name NOT ILIKE '%Administrator%' AND primary_office_other = true
  ) hardware_counts
  WHERE cnt > 0
  ON CONFLICT (system_field, system_name) DO UPDATE SET 
    institution_count = EXCLUDED.institution_count,
