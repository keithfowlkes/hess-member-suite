-- Create missing organization records for recent profiles that don't have them
...
SELECT DISTINCT ON (p.organization)
  p.organization,
  p.id,
  p.email,
  p.student_fte,
  ...
  p.created_at
FROM profiles p
LEFT JOIN organizations o ON o.contact_person_id = p.id
...
ORDER BY p.organization, p.created_at DESC;