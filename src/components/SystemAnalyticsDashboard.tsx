...
export function SystemAnalyticsDashboard() {
  ...
  // Combine key systems first, then remaining systems
  const allSystemsOrdered = selectedSystem === 'all' 
    ? [...keySystemsMap, ...SYSTEM_DATA_MAP.filter(s => !keySystemsMap.some(ks => ks.key === s.key))]
      ...
      <CardContent>
        <div className="w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {allSystemsOrdered.map(({ key, title }) => {
