  ...
  useEffect(() => {
    if (organization) {
      ...
      form.reset({
        ...
        payment_platform: profile?.payment_platform || '',
        meal_plan_management: profile?.meal_plan_management || '',
        identity_management: profile?.identity_management || '',
    ...
    try {
      if (organization) {
        ...
        if (organization.profiles?.id) {
          // Note: Organization-level fields are now stored only in the organizations table
          // Profile updates are handled separately if needed
        }
      } else {
