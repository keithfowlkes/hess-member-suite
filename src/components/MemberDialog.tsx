  ...
  useEffect(() => {
    if (member) {
      form.reset({
        ...
        membership_start_date: member.membership_start_date ? new Date(member.membership_start_date) : undefined,
        membership_end_date: member.membership_end_date ? new Date(member.membership_end_date) : undefined,
        annual_fee_amount: member.annual_fee_amount,
        notes: member.notes || '',
      });
    } else {
      form.reset({
  ...
  const onSubmit = async (data: MemberFormData) => {
    ...
    try {
      if (member) {
        ...
        await updateOrganization(member.id, updateData);
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {member ? 'Edit Member Organization' : 'Add New Member Organization'}
          </DialogTitle>
        </DialogHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Additional notes about this member..." {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
