...
const MasterDashboard = () => {
  ...
  // Fetch user role and cohort information
  useEffect(() => {
    const fetchCohortInformation = async () => {
      ...
      try {
        ...
        if (role === 'admin' || role === 'cohort_leader') {
        ...
        <main className="flex-1 p-8 space-y-8">
          ...
          <Tabs defaultValue="overview" className="space-y-6">
            <TabsList className="grid w-full grid-cols-4">
              ...
              <TabsTrigger value="cohort">Cohort Info</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="cohort" className="space-y-6">
              {currentUserRole === 'admin' || currentUserRole === 'cohort_leader' ? (
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <h2 className="text-2xl font-semibold">Cohort Information</h2>
                      <p className="text-muted-foreground">View and manage cohort leader information across the HESS Consortium</p>
                    </div>
                    <Badge variant={currentUserRole === 'admin' ? 'default' : 'secondary'}>
                      {currentUserRole === 'admin' ? 'Admin' : 'Cohort Leader'}
                    </Badge>
                  </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <p className="text-muted-foreground">
                      You don't have permission to view cohort information. This section is only available to admins and cohort leaders.
                    </p>
                  </CardContent>
                </Card>
              )}
